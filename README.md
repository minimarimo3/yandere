# Yandere Companion v0.1.20

## v0.1.20

- 日記は手動生成・自動生成のたびに `Application Support/local.yandere.companion/diaries/YYYY-MM-DD/` 配下へ時刻付きMarkdownとして永久保存します。
- GUIの「今日の日記」は従来どおり、その日に最後に生成された最新版だけを表示します。
- 手動で日記を作っていても22:30の自動日記はスキップされず、必ず一度生成されます。自動日記は `*-auto.md`、手動日記は `*-manual.md` で区別します。
- 自動日記の実行済み判定も `*-auto.md` を使うため、22:30以降にアプリを再起動しても同じ日に二重生成しません。

## v0.1.19

- 設定画面からLLMプロンプトを直接編集できるように変更。人格、観察system、観察テンプレート、自発通知、会話、日記、Cloudflare会話system、Cloudflare日記systemを個別に保存できます。
- 動的データは `{{user_text}}` / `{{snapshot}}` / `{{recent_messages}}` などのプレースホルダへ実行時に挿入します。
- プロンプト保存後は再ビルド不要で、次の会話・手動観察・日記から即時反映されます。定期観察も次回実行時に最新設定を読み込みます。
- `Yuukei Dev` 固定署名設定は維持。
...
メニューバー → 設定から **人格** と各LLMプロンプトを直接編集できます。保存後にDMGを作り直す必要はありません。

テンプレートでは `{{persona}}`, `{{user_text}}`, `{{snapshot}}`, `{{recent_messages}}` など、設定画面に表示されるプレースホルダを実行時データの挿入位置として使います。
