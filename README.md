...
- Qwen 3.8 27B / Groq が「作業中か・集中度・何をしているか・今話しかけるか」をJSONで判断
...
- 睡眠中は観察・会話・日記生成を停止し、Groq/Gemini/screenpipe検索APIを呼ばない
...
- 睡眠中: 定期観察なし、手動観察不可、チャット入力不可、Groq/Gemini呼び出しなし
...
- screenpipe の OCR / Accessibility テキストは、Qwen が「今何をしているか」を判断するため直近分だけ一時的に送信します。
...
Qwen が `should_speak=true` を返しても、アプリ側で次を強制します。
